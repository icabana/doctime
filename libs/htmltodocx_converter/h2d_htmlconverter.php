    ...
    switch ($element->tag) {
      ...
      case 'h6':
        ...
        if ($state['structure_document'] && in_array($element->tag, array('h1', 'h2', 'h3', 'h4', 'h5', 'h6')) && is_object($state['phpword_object'])) {
          ...
          $phpword_element->addTitle(htmltodocx_clean_text(utf8_encode($element->innertext)), $state['structure_headings'][$element->tag]);
          break;
        }
      ...
      break;
       ...
       case 'p':
        $state['textrun'] = $phpword_element->createTextRun();
        $state['textrun']->addText(htmltodocx_clean_text(utf8_decode($element->innertext)),  $state['current_style']);
      break;
      ...
      case 'img':
        ...
        */
          ...
          $alto = substr($element->style, 7, 3);
          $ancho = substr($element->style, 20, 3);
          
          
          
        
          $state['current_style']['height'] = $alto;
          $state['current_style']['width'] = $ancho; 
      ...
      default:
        $state['textrun'] = $phpword_element->createTextRun();
        $state['textrun']->addText(htmltodocx_clean_text(utf8_encode($element->innertext)),  $state['current_style']);
      break; 
    } 
